...
class Orbit(BaseMapper):
    ...
    def plot_fit(self, output_path=os.getcwd(), iteration=None, label=None):
        ...
        plt.plot(self.theta, self._cal_data_clean_masked, "r.", ms=0.5, alpha=0.5, label="Calibrated data")
        plt.plot(self.theta, self._zodi_data_clean_masked, "b.", ms=0.5, alpha=0.5, label="Zodi model")
        plt.legend()
    ...
    def plot_diff(self, output_path=os.getcwd()):
        theta, phi = hp.pix2ang(self._nside, self.pixel_inds_clean_masked)#, lonlat=True)
        plt.plot(self.theta, self._cal_data_clean_masked-self._zodi_data_clean_masked, "r.", ms=0.5)
        plt.title("Orbit {}".format(self.orbit_num))
        plt.xlabel("Latitude (degrees)")
        plt.ylabel("MJy/sr")
        outfile_name = (
