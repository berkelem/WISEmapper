...
class Orbit(BaseMapper):
    ...
    def apply_spline_fit(self, gain_spline, offset_spline):
        ...
        """
        gains = np.ones_like(self._cal_data_clean_masked) * gain_spline(self._orbit_mjd_clean_masked)
        # offsets = offset_spline(self._theta_ecl_clean_masked, self._orbit_mjd_clean_masked)
        offsets = np.ones_like(self._cal_data_clean_masked) * offset_spline(self._orbit_mjd_clean_masked)
    ...
    def plot_fit(self, output_path=os.getcwd(), iteration=None, label=None):
        ...
        plt.plot(self._theta_gal_clean_masked, self._cal_data_clean_masked, "r.", ms=0.5, alpha=0.5,
